...
public class GestaoLancamento {
	...
	public LancamentoDTO update(LancamentoDTO obj) {
		Lancamento entity = dao.findById(obj.getCodigo())
				.orElseThrow(() -> new BusinessException("Registros não encontrados!!!"));
//		entity.getSubitem(obj.getSubitem());
	...
	public LancamentoDTO save(LancamentoDTO obj) {
		...
		Lancamento entity = new Lancamento(obj.getCodigo(),
				new Obra(obra.get().getCodigo(), obra.get().getDescricao(), obra.get().getLocalizacao(), obra.get().getComplemento(), obra.get().getProprietario()),
				new Item(item.get().getCodigo(), item.get().getDescricao(), item.get().getComplemento()),
				new SubItem(subitem.get().getCodigo(), subitem.get().getDescricao(), subitem.get().getComplemento(), subitem.get().getItem()),
				obj.getValor(), obj.getDescricao(), obj.getDocumento(),  obj.getObservacoes()
				);
